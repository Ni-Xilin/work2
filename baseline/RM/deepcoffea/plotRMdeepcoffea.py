...
def get_tprsfprsprs_globalthr(corr_matrix, n_wins, vote_thr):
    ...
    for eta in tqdm(np.arange(-0.35, 0.8, 0.025), ascii=True, ncols=120):
        votes = np.zeros((n_test, n_test), dtype=np.int64)
        for wi in range(0, n_wins):
            corr_matrix_win = corr_matrix[n_test*wi:n_test*(wi + 1), n_test*wi:n_test*(wi + 1)]
...
best_npz_path = pathlib.Path("checkpoints/deepcoffea/deepcoffea_d3_ws5_nw11_thr20_tl500_el800_nt1000_ap1e-01_es64_lr1e-03_mep100000_bs256/deepcoffea_PatchTST_Deepcoffea_sl200_pl100_dm512_nh8_pal10_s40/RMcorrmatrix.npz")
lsetup = "_".join(best_npz_path.parent.parent.name.split("_")[-12:-9])

result_fpath = pathlib.Path("target_model/deepcoffea/dataset/CrawlE_Proc/stats_RM/lthr.p")
result_fpath2 = pathlib.Path("target_model/deepcoffea/dataset/CrawlE_Proc/stats_RM/lthr_d2.p")
...
npz_paths = [
    'checkpoints/deepcoffea/deepcoffea_d3_ws5_nw11_thr20_tl500_el800_nt1000_ap1e-01_es64_lr1e-03_mep100000_bs256/deepcoffea_PatchTST_Deepcoffea_sl200_pl100_dm512_nh8_pal10_s40/RMcorrmatrix.npz'
]

result_fpath = pathlib.Path("target_model/deepcoffea/dataset/CrawlE_Proc/stats_RM/gthr.p")
if result_fpath.exists():
    with open(result_fpath, "rb") as fp:
...
ax.set_xscale('log')
ax.set_xlim(0.0002,1)

fig.tight_layout()
fig.savefig(f"target_model/deepcoffea/dataset/CrawlE_Proc/stats_RM/dcf_eval_roc.pdf")
fig.savefig(f"target_model/deepcoffea/dataset/CrawlE_Proc/stats_RM/dcf_eval_roc.png")
...
fig.tight_layout()
fig.savefig(f"target_model/deepcoffea/dataset/CrawlE_Proc/stats_RM/dcf_eval_pr.pdf")
fig.savefig(f"target_model/deepcoffea/dataset/CrawlE_Proc/stats_RM/dcf_eval_pr.png")
