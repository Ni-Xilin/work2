...
def main(delta, win_size, n_wins, threshold, tor_len, exit_len, n_test,  emb_size,  batch_size,device):
    ...
    file_fpath = pathlib.Path("baseline/CW/CWdeepcoffea_advsamples_time0.1209_size0.0394.p")
    with open(file_fpath, "rb") as fp:
        data = pickle.load(fp)
    adv_tor_windows,  original_exit_windows,time_l2_ratios,size_l2_ratios = data["adv_tor_windows"], data["original_exit_windows"],data["avg_time_l2_ratio"],data["avg_size_l2_ratio"]
    ...
    with torch.no_grad():
        for i in tqdm(range(0, n_test, batch_size)):  
            ...
            CWxa_batch = adv_tor_windows[:,i:i+batch_size,:]  # [ n_wins, batch_size,tor_len*2]
            xp_batch = original_exit_windows[:,i:i+batch_size,:]  # [ n_wins, batch_size, exit_len*2]
            CWxa_batch = CWxa_batch.reshape(-1, tor_len*2).float().to(device)   #Gxa_batch:[n_wins,batch_size, exit_len*2] -> [batch_size*n_wins, exit_len*2]
            ...
            #CWa_out: [batch_size*n_wins, emb_size], p_out: [batch_size*n_wins, emb_size]  
            CWa_out = anchor(CWxa_batch)
            p_out = pandn(xp_batch)
            
            CWtor_embs.append(CWa_out.cpu().numpy())
            exit_embs.append(p_out.cpu().numpy())
            

        CWtor_embs = np.concatenate(CWtor_embs) # (N, emb_size)
        exit_embs = np.concatenate(exit_embs)   # (N, emb_size)
        
        CWcorr_matrix = cosine_similarity(CWtor_embs, exit_embs)

        print('Time L2 distance rate: {0}'.format(time_l2_ratios))
        print('Size L2 distance rate: {0}'.format(size_l2_ratios))
        
        np.savez_compressed("baseline/CW/deepcoffea/CWcorrmatrix.npz", corr_matrix=CWcorr_matrix,time_l2_ratios = time_l2_ratios, size_l2_ratios = size_l2_ratios)
