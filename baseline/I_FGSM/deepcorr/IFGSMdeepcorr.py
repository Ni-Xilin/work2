...
# ------------------------------------------------------------
# ------------------------------------------------------------
# 2. 核心攻击函数：I-FGSM (支持分别为time和size设置扰动大小)
# ------------------------------------------------------------
def deepcorr_ifgsm_attack(model, original_sample, time_indices, size_indices, device,
    ...
    for i in range(num_iter):
        ...
        perturbation[(sign_grads == -1) & time_mask] = epsilon_time
        ...
        perturbation[(sign_grads == -1) & size_mask] = epsilon_size
...
def main(args):
    ...
    result_fpath = pathlib.Path(f'baseline/I_FGSM/deepcorr/IFGSMdeepcorr_advsamples_time{np.mean(final_time_l2_ratios):.4f}_size{np.mean(final_size_l2_ratios):.4f}.p')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_path", type=str,default = "target_model/deepcorr/deepcorr300/tor_199_epoch23_acc0.82dict.pth", help="预训练DeepCorr模型的路径 (.pth文件)")
    ...
    parser.add_argument("--device", type=str, default="cuda:2", help="计算设备, e.g., 'cuda:0' or 'cpu'")
