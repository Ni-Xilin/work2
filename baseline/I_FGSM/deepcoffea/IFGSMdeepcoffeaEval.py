...
def main(delta, win_size, n_wins, threshold, tor_len, exit_len, n_test,  emb_size,  batch_size,device):
    ...
    file_fpath = pathlib.Path("baseline/I_FGSM/IFGSMdeepcoffea_advsamples_time0.1295_size0.0479.p")
    ...
    with torch.no_grad():
        for i in tqdm(range(0, n_test, batch_size)):  
            ...
            IFGSMxa_batch = adv_tor_windows[:,i:i+batch_size,:]  # [ n_wins, batch_size,tor_len*2]
            xp_batch = original_exit_windows[:,i:i+batch_size,:]  # [ n_wins, batch_size, exit_len*2]
            IFGSMxa_batch = IFGSMxa_batch.reshape(-1, tor_len*2).float().to(device)   #Gxa_batch:[n_wins,batch_size, exit_len*2] -> [batch_size*n_wins, exit_len*2]
            ...
            #IFGSMa_out: [batch_size*n_wins, emb_size], p_out: [batch_size*n_wins, emb_size]  
            IFGSMa_out = anchor(IFGSMxa_batch)
            p_out = pandn(xp_batch)
            
            IFGSMtor_embs.append(IFGSMa_out.cpu().numpy())
            exit_embs.append(p_out.cpu().numpy())
            

        IFGSMtor_embs = np.concatenate(IFGSMtor_embs) # (N, emb_size)
        exit_embs = np.concatenate(exit_embs)   # (N, emb_size)
        
        IFGSMcorr_matrix = cosine_similarity(IFGSMtor_embs, exit_embs)

        print('Time L2 distance rate: {0}'.format(time_l2_ratios))
        print('Size L2 distance rate: {0}'.format(size_l2_ratios))
        
        np.savez_compressed("baseline/I_FGSM/deepcoffea/IFGSMcorrmatrix.npz", corr_matrix=IFGSMcorr_matrix,time_l2_ratios = time_l2_ratios, size_l2_ratios = size_l2_ratios)
