...
adv_samples_fpath = pathlib.Path("baseline/BLANKET/BLANKETdeepcorr_time0.5238_size0.3900_add50_pad100KB_sigma50.p")
with open(adv_samples_fpath, "rb") as fp:
    # Load the adversarial samples, original samples, and perturbations:包含了一千条扰动后的测试集数据
    data = pickle.load(fp)
    total_adv_sample = data["total_adv_samples"]
...
all_outputs = []
all_labels = []
result_fpath = pathlib.Path("baseline/BLANKET/deepcorr/BLANKETtest_index300_result.p")
if False:
