...
'''
    取自github中的训练代码https://github.com/SPIN-UMass/BLANKET
'''
...
parser.add_argument('--mid', type=float, default=1, help='Train model')
parser.add_argument('--sigma', type=float, default=10, help='Train model')
parser.add_argument('--to-add', type=float, default=10, help='Train model')
...
parser.add_argument('--justpos', type=int, default=1, help='Train model')
...
device = torch.device("cuda" if use_cuda else "cpu", 2)
...
SAVE_PATH='baseline/BLANKET/model_blanketdeepcorr/mid%d_sigma%d_add%d_reg%.2f_inpsize%d_just_positive%d/'%(mid,sigma,num_to_add,reg,inpsize,args.justpos)
