...
# --------------------------------------------------------------------------
# 1. 模型与函数定义 (来自 BLANKETdeepcorr.py 和 CWdeepcorr.py)
# --------------------------------------------------------------------------
...
class TIMENOISER(nn.Module):
    ...
    def __init__(self, inp, just_positive_flag):
        ...
        self.independent = nn.Sequential(nn.Linear(inp, 500), nn.ReLU(), nn.Linear(500, inp))

    def forward(self, z_input, eps, mid, outsize=300):
...
class ADDNOISER(nn.Module):
    ...
    def __init__(self, inp, device):
        ...
        self.independent_where = nn.Sequential(nn.Linear(inp, 500), nn.ReLU(), nn.Linear(500, inp))
        self.independent_size = nn.Sequential(nn.Linear(inp, 500), nn.ReLU(), nn.Linear(500, inp))
...
class SIZEPADNOISER(nn.Module):
    ...
    def __init__(self, inp, device):
        ...
        self.inp = inp
        self.z = torch.FloatTensor(size=(1, inp)).to(device)
        self.generator = nn.Sequential(nn.Linear(inp, 500), nn.ReLU(), nn.Linear(500, inp))
...
def generate_test_samples(dataset, test_index, flow_size=300):
    """加载并格式化测试样本。"""
...
def main(args):
    ...
    test_samples = generate_test_samples(dataset, test_index)
    ...
    result_fpath = pathlib.Path(f'baseline/{result_fname}')
...
if __name__ == '__main__':
    ...
    # 路径参数
    parser.add_argument("--model_path", type=str, default="target_model/deepcorr/deepcorr300/tor_199_epoch23_acc0.82dict.pth", help="预训练的DeepCorr模型路径。")
    parser.add_argument("--blanket_model_path", type=str, default ="baseline/BLANKET/model_blanketdeepcorr/mid5_sigma50_numadd50_alladdsize100.00_reg0.00_inpsize300_just_positive1/model_epoch11_avgloss11.73", help="预训练的BLANKET生成器模型路径(.pth文件)。")
    ...
    # 评估配置参数
    parser.add_argument("--device", type=str, default="cuda:1", help="计算设备 (例如 'cuda:0' 或 'cpu')。")
    parser.add_argument("--num_samples", type=int, default=1000, help="要攻击的样本数量。")
    parser.add_argument('--input-size', type=int, default=300, help='生成器模型的输入特征向量长度。')
