...
def main():
    ...
    adv_samples_fpath = pathlib.Path("baseline/BLANKET/mdeepcorr/BLANKETdeepcorr_time0.6055_size0.2035_add50_pad100KB_sigma50.p")
    with open(adv_samples_fpath, "rb") as fp:
    # Load the adversarial samples, original samples, and perturbations:包含了一千条扰动后的测试集数据
        data = pickle.load(fp)
    adv_samples = data["total_adv_samples"]
    ...
    device = torch.device("cuda:2" if use_cuda else "cpu") # 可以选择其他cuda设备，如 cuda:1，cuda:2
    ...
    output_filename = f"BLANKETmdeepcorr_threshDC100_{threshold_dc100:.4f}.p"
    result_fpath = pathlib.Path("baseline/BLANKET/mdeepcorr") / output_filename
    results_to_save = {
