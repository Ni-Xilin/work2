...
def generate_data(dataset,train_index,flow_size):
    ...
    for i in tqdm.tqdm( train_index):
        ...
        l2s[index, 0, 0,:] = np.array(pad_or_truncate(dataset[i]['here'][0]['<-'][:flow_size],700)) * 1000.0  
        l2s[index, 0, 1,:] = np.array(pad_or_truncate(dataset[i]['there'][0]['->'][:flow_size],700)) * 1000.0  
        l2s[index, 0, 2,:] = np.array(pad_or_truncate(dataset[i]['there'][0]['<-'][:flow_size],700)) * 1000.0  
        l2s[index, 0, 3,:] = np.array(pad_or_truncate(dataset[i]['here'][0]['->'][:flow_size],700)) * 1000.0   

        l2s[index, 0, 4,:] = np.array(pad_or_truncate(dataset[i]['here'][1]['<-'][:flow_size],700)) / 1000.0 
        l2s[index, 0, 5,:] = np.array(pad_or_truncate(dataset[i]['there'][1]['->'][:flow_size],700)) / 1000.0
        l2s[index, 0, 6,:] = np.array(pad_or_truncate(dataset[i]['there'][1]['<-'][:flow_size],700)) / 1000.0
        l2s[index, 0, 7,:] = np.array(pad_or_truncate(dataset[i]['here'][1]['->'][:flow_size],700)) / 1000.0
        ...
        for idx in random_ordering:
            ...
            l2s[index,0,0,:]=np.array(pad_or_truncate(dataset[idx]['here'][0]['<-'][:flow_size],700))*1000.0
            l2s[index,0,1,:]=np.array(pad_or_truncate(dataset[i]['there'][0]['->'][:flow_size],700))*1000.0
            l2s[index,0,2,:]=np.array(pad_or_truncate(dataset[i]['there'][0]['<-'][:flow_size],700))*1000.0
            l2s[index,0,3,:]=np.array(pad_or_truncate(dataset[idx]['here'][0]['->'][:flow_size],700))*1000.0

            l2s[index,0,4,:]=np.array(pad_or_truncate(dataset[idx]['here'][1]['<-'][:flow_size],700))/1000.0
            l2s[index,0,5,:]=np.array(pad_or_truncate(dataset[i]['there'][1]['->'][:flow_size],700))/1000.0
            l2s[index,0,6,:]=np.array(pad_or_truncate(dataset[i]['there'][1]['<-'][:flow_size],700))/1000.0
            l2s[index,0,7,:]=np.array(pad_or_truncate(dataset[idx]['here'][1]['->'][:flow_size],700))/1000.0
...
#         self.d = nn.Dropout2d()
    ...
    def forward(self, inp,eps,mid,outsize = 700):
        x = inp[:,:self.inp]
...
def train_adv(adv_model, size_add_model, size_pad_model, optim_adv, disc_model, opt_dis, model, device, data, label,
              num_to_add=0, reg=0, eps=0.3, mid=5.0, deps=700,
...
def gen_advs(adv_model,optim_adv,disc_model,opt_dis, model, device, data, label, lr, alpha=0.5, eps=0.3,mid=5.0,deps = 700):
    model.eval()
    model.zero_grad()
...
if TRAINING:
    ...
    test_index=pickle.load(open('target_model/mdeepcorr/test_index.pickle'))
...
parser.add_argument('--input-size', type=int, default=700,
                    help='输入流量特征向量的长度 (代码中针对DeepCorr设为700)。')
...
model = Net().to(device)
LOADED=torch.load('target_model/deepcorr/deepcorr700/tor700_199_epoch11_acc0.88.pth', map_location=device)
...
l2s,labels=generate_data(dataset=dataset,train_index=train_index,flow_size=flow_size)
...
SAVE_PATH='baseline/BLANKET/model_blanketmdeepcorr/mid%d_sigma%d_numadd%d_alladdsize%.2f_reg%.2f_inpsize%d_just_positive%d/'%(mid,sigma,num_to_add,size_padding_total_kb,reg,inpsize,args.justpos)
