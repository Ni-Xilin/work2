...
def main(delta, win_size, n_wins, threshold, tor_len, exit_len, n_test,  emb_size,  batch_size,device):
    ...
    file_fpath = pathlib.Path("baseline/BLANKET/BLANKETdeepcoffea_time0.1188_size0.2293.p")
    with open(file_fpath, "rb") as fp:
        data = pickle.load(fp)
    adv_tor_windows,  original_exit_windows,time_l2_ratios,size_l2_ratios = data["adv_tor_windows"], data["original_exit_windows"],data["time_ratios"],data["size_ratios"]
    ...
    with torch.no_grad():
        for i in tqdm(range(0, n_test, batch_size)):  
            ...
            BLANKETxa_batch = adv_tor_windows[:,i:i+batch_size,:]  # [ n_wins, batch_size,tor_len*2]
            xp_batch = original_exit_windows[:,i:i+batch_size,:]  # [ n_wins, batch_size, exit_len*2]
            BLANKETxa_batch = BLANKETxa_batch.reshape(-1, tor_len*2).float().to(device)   #Gxa_batch:[n_wins,batch_size, exit_len*2] -> [batch_size*n_wins, exit_len*2]
            ...
            #BLANKETa_out: [batch_size*n_wins, emb_size], p_out: [batch_size*n_wins, emb_size]  
            BLANKETa_out = anchor(BLANKETxa_batch)
            p_out = pandn(xp_batch)
            
            BLANKETtor_embs.append(BLANKETa_out.cpu().numpy())
            exit_embs.append(p_out.cpu().numpy())
            

        BLANKETtor_embs = np.concatenate(BLANKETtor_embs) # (N, emb_size)
        exit_embs = np.concatenate(exit_embs)   # (N, emb_size)
        
        BLANKETcorr_matrix = cosine_similarity(BLANKETtor_embs, exit_embs)

        print('Time L2 distance rate: {0}'.format(np.mean(time_l2_ratios)))
        print('Size L2 distance rate: {0}'.format(np.mean(size_l2_ratios)))
        
        np.savez_compressed("baseline/BLANKET/deepcoffea/BLANKETcorrmatrix.npz", corr_matrix=BLANKETcorr_matrix,time_l2_ratios = time_l2_ratios, size_l2_ratios = size_l2_ratios)
