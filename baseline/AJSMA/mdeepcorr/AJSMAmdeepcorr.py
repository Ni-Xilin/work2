...
def jsma_attack(model, original_sample, target_label, 
    ...
    for i in range(max_features_to_perturb):
        ...
        saliency_map[original_sample.squeeze() <= 0] = -float('inf')
...
def evaluate_jsma_on_test_set(model, test_loader, device, attack_params):
    ...
    for (original_batch, labels_batch) in tqdm(test_loader, desc="正在攻击测试集"):
        for i in range(original_batch.size(0)):
            ...
            with torch.no_grad():
                original_output = model(original_sample,dropout=0.0)
    ...
    result_fpath = pathlib.Path(f'baseline/AJSMA/mdeepcorr/AJSMAmdeepcorr_advsamples_time{avg_time_l2_ratio:.4f}_size{avg_size_l2_ratio:.4f}.p')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--max_features_to_perturb", type=int, default=700, help="最大修改特征点数")
    ...
    test_l2s, test_labels = generate_positive_samples_only(dataset, test_index, flow_size=700)
