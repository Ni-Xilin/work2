...
def main():
    ...
    adv_samples_fpath = pathlib.Path("baseline/AJSMA/AJSMAdeepcorr_advsamples_time0.1469_size0.0618.p")
    with open(adv_samples_fpath, "rb") as fp:
    # Load the adversarial samples, original samples, and perturbations:包含了一千条扰动后的测试集数据
        data = pickle.load(fp)
    adv_samples = data["total_adv_sample"]
    ...
    output_filename = f"AJSMAmdeepcorr_threshDC100_{threshold_dc100:.4f}.p"
    result_fpath = pathlib.Path("baseline/AJSMA/mdeepcorr") / output_filename
    results_to_save = {
