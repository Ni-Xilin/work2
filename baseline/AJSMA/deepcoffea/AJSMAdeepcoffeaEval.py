...
def main(delta, win_size, n_wins, threshold, tor_len, exit_len, n_test,  emb_size,  batch_size,device):
    ...
    file_fpath = pathlib.Path("baseline/AJSMA/AJSMAdeepcoffea_advsamples_time0.0987_size0.0243.p")
    ...
    with torch.no_grad():
        for i in tqdm(range(0, n_test, batch_size)):  
            ...
            AJSMAxa_batch = adv_tor_windows[:,i:i+batch_size,:]  # [ n_wins, batch_size,tor_len*2]
            xp_batch = original_exit_windows[:,i:i+batch_size,:]  # [ n_wins, batch_size, exit_len*2]
            AJSMAxa_batch = AJSMAxa_batch.reshape(-1, tor_len*2).float().to(device)   #Gxa_batch:[n_wins,batch_size, exit_len*2] -> [batch_size*n_wins, exit_len*2]
            ...
            #AJSMAa_out: [batch_size*n_wins, emb_size], p_out: [batch_size*n_wins, emb_size]  
            AJSMAa_out = anchor(AJSMAxa_batch)
            p_out = pandn(xp_batch)
            
            AJSMAtor_embs.append(AJSMAa_out.cpu().numpy())
            exit_embs.append(p_out.cpu().numpy())
            

        AJSMAtor_embs = np.concatenate(AJSMAtor_embs) # (N, emb_size)
        exit_embs = np.concatenate(exit_embs)   # (N, emb_size)
        
        AJSMAcorr_matrix = cosine_similarity(AJSMAtor_embs, exit_embs)

        print('Time L2 distance rate: {0}'.format(time_l2_ratios))
        print('Size L2 distance rate: {0}'.format(size_l2_ratios))
        
        np.savez_compressed("baseline/AJSMA/deepcoffea/AJSMAcorrmatrix.npz", corr_matrix=AJSMAcorr_matrix,time_l2_ratios = time_l2_ratios, size_l2_ratios = size_l2_ratios)
