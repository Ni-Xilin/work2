...
class TorchTrainer:
    ...
    def train(self) -> dict:
        ...
        for epoch in range(1, self.config.train_epochs + 1):
            ...
            for step, batch in enumerate(self.train_loader, start=1):
                ...
                epoch_metrics["original_accuracy"] += float(outputs["original_accuracy"])
                epoch_metrics["adv_accuracy"] += float(outputs["adv_accuracy"])
                epoch_metrics["accuracy_drop"] += float(outputs["accuracy_drop"])
                epoch_metrics["attack_success_rate"] += float(outputs["attack_success_rate"])
                epoch_metrics["flip_rate"] += float(outputs["flip_rate"])
                epoch_metrics["mean_original_logit"] += float(outputs["mean_original_logit"])
                epoch_metrics["mean_adv_logit"] += float(outputs["mean_adv_logit"])
                epoch_metrics["mean_original_prob"] += float(outputs["mean_original_prob"])
                epoch_metrics["mean_adv_prob"] += float(outputs["mean_adv_prob"])
                steps += 1
                ...
                if step % self.config.log_interval == 0:
                    print(
                        ...
                        f"orig_acc={outputs['original_accuracy']:.4f} "
                        f"adv_acc={outputs['adv_accuracy']:.4f} "
                        f"acc_drop={outputs['accuracy_drop']:.4f} "
            ...
            print(
                ...
                f"train_orig_acc={train_summary['original_accuracy']:.4f} "
                f"train_adv_acc={train_summary['adv_accuracy']:.4f} "
                f"eval_orig_acc={eval_summary['original_accuracy']:.4f} "
                f"eval_adv_acc={eval_summary['adv_accuracy']:.4f} "
                f"eval_acc_drop={eval_summary['accuracy_drop']:.4f} "
    ...
    def evaluate(self) -> dict:
        ...
        with self.torch.no_grad():
            for step, batch in enumerate(self.eval_loader, start=1):
                ...
                metrics["original_accuracy"] += float(outputs["original_accuracy"])
                metrics["adv_accuracy"] += float(outputs["adv_accuracy"])
                metrics["accuracy_drop"] += float(outputs["accuracy_drop"])
                metrics["attack_success_rate"] += float(outputs["attack_success_rate"])
                metrics["flip_rate"] += float(outputs["flip_rate"])
                metrics["mean_original_logit"] += float(outputs["mean_original_logit"])
                metrics["mean_adv_logit"] += float(outputs["mean_adv_logit"])
                metrics["mean_original_prob"] += float(outputs["mean_original_prob"])
                metrics["mean_adv_prob"] += float(outputs["mean_adv_prob"])
                steps += 1
    ...
    def _forward_batch(self, batch, log_shapes: bool):
        ...
        with self.torch.no_grad():
            original_target_logits = self.target_model.forward(target_original_flow).to(self.trainable_device)
        ...
        metric_outputs = self._compute_attack_metrics(original_target_logits=original_target_logits, adv_target_logits=target_logits)
        accuracy_outputs = self._compute_accuracy_metrics(
        ...
        return {
            ...
            "original_accuracy": accuracy_outputs["original_accuracy"],
            "adv_accuracy": accuracy_outputs["adv_accuracy"],
            "accuracy_drop": accuracy_outputs["accuracy_drop"],
    ...
    def _compute_attack_metrics(self, original_target_logits, adv_target_logits) -> dict[str, float]:
        ...
        original_positive = original_logits > 0
        adv_positive = adv_logits > 0
        ...
        return {
            ...
            "mean_original_prob": float(self.torch.sigmoid(original_logits).mean().item()),
            "mean_adv_prob": float(self.torch.sigmoid(adv_logits).mean().item()),
        }

    def _compute_accuracy_metrics(self, original_target_logits, adv_target_logits) -> dict[str, float]:
        original_probs = self.torch.sigmoid(original_target_logits.detach().reshape(-1))
        adv_probs = self.torch.sigmoid(adv_target_logits.detach().reshape(-1))
        threshold = 0.1
        original_accuracy = float((original_probs > threshold).float().mean().item())
        adv_accuracy = float((adv_probs > threshold).float().mean().item())
        return {
            "original_accuracy": original_accuracy,
            "adv_accuracy": adv_accuracy,
            "accuracy_drop": float(original_accuracy - adv_accuracy),
        }
