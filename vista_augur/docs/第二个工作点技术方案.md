...
- **数据集不能变。** 第二个工作点必须继续使用第一工作点已经采用、现已集中存放在 `datasets/` 下的同一批数据，而不能为了适配新模型去另找一套更“顺手”的数据。原因很直接：只要数据变了，后续实验即便结果更好，也无法严格说明提升究竟来自模型结构，还是来自数据分布变化。保持数据不变，等于锁定了实验地基，使得后续改动能够被合理归因。
...
- **DeepCorr / mDeepCorr 数据保持不变。** 这部分数据位于 `datasets/deepcorr/`，在现有工程里已经被整理成 8 通道张量。第二工作点不会去改这些数据的定义，只会继续沿用 Tor 侧有效的 4 个通道，也就是当前生成器真正修改的那些位置。这样做的好处是，DeepCorr300 和 mDeepCorr 的实验能够与第一工作点逐项对齐，而不是只做概念对齐。

- **DeepCoFFEA 数据保持不变。** 这部分数据位于 `datasets/deepcoffea/CrawlE_Proc`，其任务形态是 session 级 IPD 和 size 序列，再经过窗口划分和 embedding 匹配进入目标模型。第二工作点在这条链路上同样不改变数据协议，而只是替换生成器的内部结构，使其仍然能够输出未来窗口上的扰动。
...
| --- | --- | --- | --- |
| DeepCorr300 / mDeepCorr | `datasets/deepcorr/` | 8 通道固定长度流量张量 | `seq_len=96`, `pred_len=48` |
| DeepCoFFEA | `datasets/deepcoffea/CrawlE_Proc` | session 级 IPD/size 序列 | `seq_len=150`, `pred_len=70` |
