...
```python
def reprogram_features(x_ts, x_vis, prompt_ids, prototypes):
    """
    ...
    prompt_ids : Tensor
        分词后的流量提示词序列。
        这一分支天然属于语言空间，因此不经过重编程层，而是直接嵌入。
    prototypes : Tensor
        共享语义原型矩阵，形状为 [K, d]。
        如果 K == 0，则映射失去意义，函数必须立即终止。
    ...
    # 再计算它们对共享原型空间的响应权重
    a_ts = softmax((h_ts @ prototypes.T) / sqrt(prototypes.shape[-1]), dim=-1)
    a_vis = softmax((h_vis @ prototypes.T) / sqrt(prototypes.shape[-1]), dim=-1)

    # 用响应权重重组原型，得到可送入 LLM 的 token
    z_ts = a_ts @ prototypes
    z_vis = a_vis @ prototypes

    # 文本提示分支直接嵌入，作为语言语义锚点
    e_text = frozen_embed(prompt_ids)
