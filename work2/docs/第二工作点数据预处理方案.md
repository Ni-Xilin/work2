...
> [!IMPORTANT]
> 这份文档只回答一个问题：  
> **从 `Generator_Trainer/` 现有真实数据出发，怎样整理出第二工作点的三类输入：时序输入、视觉化结构输入、prompt 语义输入。**
>
...
- 这份方案依赖的既有事实主要来自以下代码路径：
  - `Generator_Trainer/data_provider/data_loader.py`
  - `Generator_Trainer/exp/exp_main_deepcorr300.py`
  - `Generator_Trainer/exp/exp_main_deepcoffea.py`
  - `Generator_Trainer/generator/generator.py`
...
第二工作点的数据预处理不是重新发明一套新数据协议，而是把 `Generator_Trainer/` 中已经存在的真实历史流量，稳定整理成三类彼此分工明确的输入：
